...
class NetMessenger(Messenger):
    ...
    def prepare(self, message, sentArgs=None, senderChannel=None):
        """
        ...
        # From:
        datagram.addChannel(senderChannel or self.air.ourChannel)

        messageType = self.__message2type[message]
        datagram.addUint16(messageType)
        datagram.addBlob(msgpack.packb(sentArgs))
    ...
    def send(self, message, sentArgs=None):
        """
    ...
    def handle(self, msgType, di):
        ...
        if self.air.getMsgSender() == self.air.ourChannel:
            return
        ...
        sentArgs = msgpack.unpackb(di.getBlob())

        if type(sentArgs) is not list:
            self.notify.warning(
                "Received non-list item in %s message: %r" % (message, sentArgs)
            )
            return
