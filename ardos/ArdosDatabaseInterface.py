...
class ArdosDatabaseInterface:
    """
    This class is part of Panda3D's new MMO networking framework.
    It interfaces with Ardos database(s) to manage objects directly, rather than
    via DB-StateServers.

    Do not create this class directly; instead, use ArdosInternalRepository's
    dbInterface attribute.
    """

    notify = DirectNotifyGlobal.directNotify.newCategory("ArdosDatabaseInterface")
    ...
    def createObject(self, databaseId, dclass, fields={}, callback=None):
        ...
        for k, v in fields.items():
            ...
            if not field:
                self.notify.error(
                    "Creation request for %s object contains "
    ...
    def handleCreateObjectResp(self, di):
        ...
        if ctx not in self._callbacks:
            self.notify.warning(
                "Received unexpected DBSERVER_CREATE_OBJECT_RESP"
    ...
    def queryObject(self, databaseId, doId, callback, dclass=None, fieldNames=()):
        ...
        if not fieldNames:
            ...
            if len(fieldNames) > 1:
                dg.addServerHeader(
                    databaseId, self.air.ourChannel, DBSERVER_OBJECT_GET_FIELDS
                )
            else:
                dg.addServerHeader(
                    databaseId, self.air.ourChannel, DBSERVER_OBJECT_GET_FIELD
                )
        ...
        for fieldName in fieldNames:
            ...
            if field is None:
                self.notify.error(
                    "Bad field named %s in query for"
    ...
    def handleQueryObjectResp(self, msgType, di):
        ...
        if ctx not in self._callbacks:
            self.notify.warning(
                "Received unexpected %s" " (ctx %d)" % (MsgId2Names[msgType], ctx)
            )
            return
        ...
        try:
            ...
            for x in range(fieldCount):
                ...
                if not field:
                    self.notify.error(
                        "Received bad field %d in query for"
    ...
    def updateObject(
        self, databaseId, doId, dclass, newFields, oldFields=None, callback=None
    ):
        """
        ...
        for k, v in newFields.items():
            ...
            if not field:
                self.notify.error(
                    "Update for %s(%d) object contains invalid"
        ...
        if oldFields is not None:
            ...
            if fieldCount == 1:
                dg.addServerHeader(
                    databaseId, self.air.ourChannel, DBSERVER_OBJECT_SET_FIELD_IF_EQUALS
                )
            else:
                dg.addServerHeader(
                    databaseId,
            ...
            if fieldCount == 1:
                dg.addServerHeader(
                    databaseId, self.air.ourChannel, DBSERVER_OBJECT_SET_FIELD
                )
            else:
                dg.addServerHeader(
                    databaseId, self.air.ourChannel, DBSERVER_OBJECT_SET_FIELDS
                )
        dg.addUint32(doId)
        if fieldCount != 1:
            dg.addUint16(fieldCount)
    ...
    def handleUpdateObjectResp(self, di, multi):
        ...
        try:
            ...
            for x in range(fieldCount):
                ...
                if not field:
                    self.notify.error(
                        "Received bad field %d in update"
